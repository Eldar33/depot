...
class SupportMailboxTest < ActionMailbox::TestCase
  ...
  test "we create a SupportRequest with the most recent order" do
    ...
    sr = SupportRequest.last
    assert_equal recent_order.email, sr.email
    assert_equal "Need help", sr.subject
    assert_equal "I can't figure out how to check out!!", sr.body
    assert_equal recent_order, sr.order
  end
end
